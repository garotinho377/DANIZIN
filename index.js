	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
					...
					levelnol: '*error* 0 °-°',
					error: {
				stick: 'Falhou,Tente Novamente Mais tarde ^_^*',
				...
				only: {
					...
					ownerG: '[❗] Danizin? Este é um recurso especial para o Danizin ❌',
					ownerB: '[❗] Danizin? Este é um recurso especial para o Danizin ❌',
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado como amigo de DANIZIN. Registre-se para fazer amizade com o bot Hypezin
		...
		if (messagesC.includes("bot")){
			...
			switch(command) {
				...
				case 'ssweb':
					...
					if (args.length < 1) return reply('Cadê O Url Men?')
					tekss = body.slice(7)
					reply(mess.wait)
					...
					break
                case 'ytkomen':
					if (args.length < 1) return reply('Cadê o Texto Men?')
					gh = body.slice(9)
					usnm = gh.split("|")[0];
					cmn = gh.split("|")[1];
...
break
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOS BOT HYPEZIN 👑* 」\n\`\`\``
					no = 0
...
// recursos anime, random @Causs domina vida
          ...
          reply('Espere..')
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT HYPEZIN ✨* 」\n*│+ wa.me/994407645278*╰──────*「 *DANIZIN* 」*────*\n\n*_SE QUER SER ADMIN DO BOT HYPEZIN_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
				...
				case 'bomdia':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Bom dia, Vcs Sao Fodaah ❤️'})
					break
				case 'boatarde':
					memein = await kagApi.memeindo()
        ...
        case 'promover':
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						...
						reply('1 para ativar, 0 para desativar, lerdao em KKKKK')
					}
					break
				case 'bemvindo':
					if (!isGroup) return reply(mess.only.group)
